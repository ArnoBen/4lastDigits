...
for i in range(10):
    ...
    algo_result = last_4_digits_pow(random_n)
    print("n =",random_n)
...
def actual_solution(n):
    n = np.float64(n)
    intermediate_result = n*np.log10(n)
    decimal_part = intermediate_result - np.floor(intermediate_result)
    decimal_part
    result = np.power(10,decimal_part) * (10**np.floor(intermediate_result))
    result
    result = result%10000
    return result
print(actual_solution(124))